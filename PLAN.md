...
2. `Phase 1: 数据与回测底座`
   - 状态：`进行中`
   - 剩余工作：历史行情采集还偏 `smoke-test / on-demand`，同步编排、覆盖范围、长期完整性校验还需要继续补。
   - 打通 Futu 行情采集、历史数据落地、统一标的主数据。
   - 完成股票/ETF/期权的事件驱动回测器、交易成本模型、组合记账。
3. `Phase 2: 策略研发与筛选`
   - 状态：`进行中`
   - 剩余工作：策略实现与研究报告基线已在，但更完整的真实历史覆盖、容量分析和稳定筛选结果还需要沉淀。
   - 按上面的 3 个策略做参数最少化实现。
...
4. `Phase 3: 模拟交易与人工确认链路`
   ...
   - 剩余工作：港美连续 6 周纸面 + A 股 4 周建议单的实际运行证据。
5. `Phase 4: 小资金实盘`
   - 状态：`受时间阻塞 · 自动门禁`
   - 剩余工作：`10% -> 30% -> 100%` 的资金分阶段上线和 `4 周 / 8 周` 稳定性验收，必须依赖真实时间推进。
...
6. `Phase 5: 二期扩展`
   - 状态：`延期`
   - 剩余工作：这部分按计划本来就要求 V1 稳定盈利后再开始，不属于当前一次性交付能完成的范围。
...
- 进度快照
  ...
  - 集成测试：`已完成`
  - 回测可复现性：`已完成`
  - 纸面交易验收：`每日自动采集，等待自然时间证据`
  - 实盘交易验收：`由 pass-streak 自动门禁，等待自然时间证据`
