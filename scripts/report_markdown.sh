...
if acceptance_file is not None:
    ...
    print(f"- Ready Weeks: `{acceptance.get('ready_weeks')}`")
    print(f"- Current Clean Week Streak: `{evidence.get('current_clean_week_streak')}`")
    print(f"- Blocked Days: `{evidence.get('blocked_days')}`")
