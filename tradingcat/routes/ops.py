...
def get_risk_config(request: Request):
    return get_app_state(request).risk._config.model_dump(mode="json")
...
def ops_execution_metrics(request: Request):
    return get_app_state(request).operations_execution_metrics()


@router.get("/daily-report")
def ops_daily_report(request: Request):
    return get_app_state(request).operations_period_report(window_days=1, label="daily")


@router.get("/weekly-report")
def ops_weekly_report(request: Request):
    return get_app_state(request).operations_period_report(window_days=7, label="weekly")


@router.get("/postmortem")
def ops_postmortem(request: Request, window_days: int = 7):
    return get_app_state(request).operations_postmortem(window_days)


@router.get("/incidents/replay")
def ops_incidents_replay(request: Request, window_days: int = 7):
    return get_app_state(request).incident_replay(window_days)


@router.post("/journal/record")
def ops_journal_record(request: Request):
    return get_app_state(request).record_operations_journal()
...
def ops_rollout(request: Request):
    return get_app_state(request).operations_rollout()
...
def ops_rollout_checklist(request: Request, stage: str | None = None, as_of: date | None = None):
    return get_app_state(request).rollout_checklist(stage, as_of)
...
def ops_rollout_policy(request: Request):
    return get_app_state(request).rollout_policy_summary()
...
def ops_apply_rollout_policy_recommendation(request: Request):
    ...
    return app.rollout_policy.apply_recommendation(app.operations_rollout())
...
def ops_go_live(request: Request, as_of: date | None = None):
    return get_app_state(request).go_live_summary(as_of)


@router.get("/live-acceptance")
def ops_live_acceptance(request: Request, as_of: date | None = None, incident_window_days: int = 14):
    return get_app_state(request).live_acceptance_summary(as_of, incident_window_days)
